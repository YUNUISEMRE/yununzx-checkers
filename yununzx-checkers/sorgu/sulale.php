...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!tc_kontrol($tc)) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, API_URL . '?path=sulale.php');
        ...
        if ($data && $data['ok'] && !empty($data['data']['results'])) {
            $sonuc = $data['data']['results'];
            sorgu_log($_SESSION['user_id'], 'Sülale', $tc, $sonuc);
            if (!isVip()) sorgu_hakki_azalt($_SESSION['user_id']);
        } else {
            $hata = '🌸 Sülale bilgisi bulunamadı.';
        }
    }
}
?>
...
<head>
    ...
    <title>🌸 Sülale Sorgu | Yununzx</title>
...
<body>
    ...
    <div class="main-content" style="max-width:700px;margin:80px auto 0;">
        <div class="card">
            <div class="card-title"><i class="fas fa-tree"></i> Sülale Sorgu</div>
        ...
        <?php if ($sonuc): ?>
            <div class="card" style="border-color:var(--sakura-pink);">
                <div class="card-title"><i class="fas fa-cherry-blossom"></i> Sülale Bilgileri</div>
                <div class="sonuc-card">
                    <?php foreach ($sonuc as $kisi): ?>
                        <div style="padding:6px 0;border-bottom:1px solid var(--border);">
                            <?= htmlspecialchars($kisi['AD'] ?? '') . ' ' . htmlspecialchars($kisi['SOYAD'] ?? '') ?>
                            <?= isset($kisi['TC']) ? ' (TC: ' . $kisi['TC'] . ')' : '' ?>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
