...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!tc_kontrol($tc)) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, API_URL . '?path=vesika.php');
        ...
        if ($data && $data['ok'] && !empty($data['data']['results'])) {
            ...
            sorgu_log($_SESSION['user_id'], 'Vesika', $tc, $sonuc);
            if (!isVip()) sorgu_hakki_azalt($_SESSION['user_id']);
        } else {
            $hata = '🌸 Vesika fotoğrafı bulunamadı.';
        }
    }
}
?>
...
<head>
    ...
    <title>🌸 Vesika Sorgu | Yununzx</title>
...
<body>
    ...
    <div class="main-content" style="max-width:700px;margin:80px auto 0;">
        <div class="card">
            <div class="card-title"><i class="fas fa-camera"></i> Vesika Sorgu</div>
        ...
        <?php endif; ?>
        <?php if ($sonuc && isset($sonuc['FOTOGRAF_URL'])): ?>
            <div class="card" style="border-color:var(--sakura-pink);text-align:center;">
                <div class="card-title"><i class="fas fa-cherry-blossom"></i> Vesika Fotoğrafı</div>
                <img src="<?= htmlspecialchars($sonuc['FOTOGRAF_URL']) ?>" alt="Vesika" style="max-width:300px;border-radius:16px;border:3px solid var(--sakura-pink);">
            </div>
        <?php endif; ?>
    </div>
