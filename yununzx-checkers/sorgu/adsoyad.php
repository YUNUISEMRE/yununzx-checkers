...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!tc_kontrol($tc)) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, API_URL . '?path=adsoyad.php');
        ...
        if ($data && $data['ok'] && !empty($data['data']['results'])) {
            ...
            sorgu_log($_SESSION['user_id'], 'AdSoyad', $tc, $sonuc);
            if (!isVip()) sorgu_hakki_azalt($_SESSION['user_id']);
        } else {
            $hata = '🌸 Ad soyad bilgisi bulunamadı.';
        }
    }
}
?>
...
<head>
    ...
    <title>🌸 Ad Soyad Sorgu | Yununzx</title>
...
<body>
    ...
    <div class="main-content" style="max-width:700px;margin:80px auto 0;">
        <div class="card">
            <div class="card-title"><i class="fas fa-user-tag"></i> Ad Soyad Sorgu</div>
