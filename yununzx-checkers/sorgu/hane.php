...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!tc_kontrol($tc)) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, API_URL . '?path=hane.php');
        ...
        if ($data && $data['ok'] && !empty($data['data']['results'])) {
            ...
            sorgu_log($_SESSION['user_id'], 'Hane', $tc, $sonuc);
            if (!isVip()) sorgu_hakki_azalt($_SESSION['user_id']);
        } else {
            $hata = '🌸 Hane bilgisi bulunamadı.';
        }
    }
}
?>
...
<head>
    ...
    <title>🌸 Hane Sorgu | Yununzx</title>
...
<body>
    ...
    <div class="main-content" style="max-width:700px;margin:80px auto 0;">
        <div class="card">
            <div class="card-title"><i class="fas fa-building"></i> Hane Sorgu</div>
        ...
        <?php if ($sonuc): ?>
            <div class="card" style="border-color:var(--sakura-pink);">
                <div class="card-title"><i class="fas fa-cherry-blossom"></i> Hane Bilgileri</div>
